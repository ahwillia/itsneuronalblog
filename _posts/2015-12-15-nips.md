...
> The [convex envelope]() of a nonconvex function can be optimized efficiently, and has a unique solution that is equal to the global minimum of the original function. The problem is that it is generally intractable to compute. [Mobahi and Fisher (2015)](http://people.csail.mit.edu/hmobahi/pubs/gaussian_convenv_2015.pdf) show that, for certain problems,{%include footnote.html n=1 %} a Gaussian smoothing of the nonconvex function can be found in closed form and is the best affine approximation of the convex envelope. You can first solve a *very smoothed* version of the nonconvex problem, and then solve progressively less smoothed versions of the problem (i.e. use continuation methods). The work seems of more theoretical interest rather than practical use at the moment (at least for high-dimensional problems), but it will be interesting to see if these ideas lead to new algorithms.
